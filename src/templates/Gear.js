...
const Gear = (props) => {
	...
	const prevPage = currentPage - 1 === 1 ? '/category/gear' : `/category/gear/${currentPage - 1}`;
	const nextPage = `/category/gear/${currentPage + 1}`;

	return (
		<Layout>
			<SEO title='Gear' keywords={[ 'dj', 'dj blog', 'dj equipment', 'dj gear', 'gear' ]} />
			<Nav />
			<header>
...
export const pageQuery = graphql`
	query GearQuery($skip: Int!, $limit: Int!) {
		allContentfulBlog(
			sort: { fields: [createdAt], order: DESC }
			filter: { node_locale: { eq: "en-US" }, category: { elemMatch: { title: { eq: "Gear" } } } }
			skip: $skip
			limit: $limit
		) {
			edges {
