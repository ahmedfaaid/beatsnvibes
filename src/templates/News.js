...
const News = (props) => {
	...
	const prevPage = currentPage - 1 === 1 ? '/category/news' : `/category/news/${currentPage - 1}`;
	const nextPage = `/category/news/${currentPage + 1}`;

	return (
		<Layout>
			<SEO title='News' keywords={[ 'dj', 'dj blog', 'dj equipment', 'dj news', 'opinions' ]} />
			<Nav />
			<header>
...
export const pageQuery = graphql`
	query NewsQuery($skip: Int!, $limit: Int!) {
		allContentfulBlog(
			...
			filter: { node_locale: { eq: "en-US" }, category: { elemMatch: { title: { eq: "News" } } } }
			skip: $skip
			limit: $limit
		) {
			edges {
